...
export async function GET() {
    ...
    try {
        ...
        console.log(`[NEXT-API-TIME] GET /api/settings status=${status} duration=${(performance.now() - start).toFixed(1)}ms fetch=${fetchMs.toFixed(1)}ms backend=${BACKEND}`);
    }
}
...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        console.log(`[NEXT-API-TIME] POST /api/settings status=${status} duration=${(performance.now() - start).toFixed(1)}ms fetch=${fetchMs.toFixed(1)}ms backend=${BACKEND}`);
    }
}
