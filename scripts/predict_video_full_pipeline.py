...
class IncrementalJsonSaver:
    """Save prediction_results.json (track_votes only) when:
    - A new track_id is seen for the first time
    - Every `interval` frames per track_id
    - When a track_id disappears from the current frame

    Only writes tracks that have a non-empty stable_label from voting.
    The final save_json_only() at the end overwrites with full frames + metadata.
    """

    def __init__(self, output_dir: Path, data: dict, interval: int = 100) -> None:
        self.output_dir = output_dir
        self.data = data  # live reference — metadata updates and frame appends are visible automatically
        self.interval = interval
        self._seen_ids: set[int] = set()
        self._last_save_frame: dict[int, int] = {}
        self._prev_frame_ids: set[int] = set()
        self._track_votes: dict[int, dict] = {}
    ...
    def update(self, frame_index: int) -> None:
        ...
        current_ids = {int(p["id"]) for p in current_frame.get("persons", [])}
        ...
        if new_ids:
            ...
            for tid in new_ids:
                self._last_save_frame[tid] = frame_index
            should_save = True

        # every interval frames per active track
        for tid in current_ids:
            last = self._last_save_frame.get(tid, frame_index)
            if frame_index - last >= self.interval:
                self._last_save_frame[tid] = frame_index
                should_save = True

        # track disappeared (last frame for that track)
        gone_ids = self._prev_frame_ids - current_ids
        if gone_ids:
            should_save = True
        ...
        if should_save:
            # update latest stable vote per track_id from current frame's persons
            for person in current_frame.get("persons", []):
                tid = int(person["id"])
                stable = person.get("stable_clothing") or {}
                label = stable.get("label") or ""
                if label:
                    self._track_votes[tid] = {
                        "stable_label": label,
                        "stable_classes": stable.get("classes") or [],
                    }
            self._write()

    def _write(self) -> None:
        # write only tracks that have a voted class name — skip tracks with no result yet
        votes_with_result = {
            str(tid): vote
            for tid, vote in self._track_votes.items()
            if vote.get("stable_label")
        }
        if not votes_with_result:
            return
        payload = {
            "metadata": self.data.get("metadata", {}),
            "track_votes": votes_with_result,
        }
        path = self.output_dir / "prediction_results.json"
...
def predict_full(args: argparse.Namespace) -> tuple[dict[str, Any], Timings]:
    ...
    incremental_json_saver = IncrementalJsonSaver(output_dir, data, interval=100)
    detections_count = 0
    ...
    for frame_index, frame in frame_iter:
        ...
        incremental_json_saver.update(frame_index)
    ...
    data["metadata"] = metadata  # replace partial metadata with full metadata; frames reference unchanged
